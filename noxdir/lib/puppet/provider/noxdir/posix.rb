Puppet::Type.type(:noxdir).provide :posix do
  ...
  def create
    FileUtils.mkdir resource[:path], :mode => resource[:mode].to_i(8)
  end

  def destroy
    FileUtils.rmdir resource[:path]
  end
  ...
  def owner=(should)
    begin
      File.chown(should, nil, resource[:path])
    rescue => detail
  ...
  def group=(should)
    begin
      File.chown(nil, should, resource[:path])
    rescue => detail
